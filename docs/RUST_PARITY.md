...
| Ed25519 keypair/signature | Interoperable; verifier divergence documented | generation; Rust/wallet byte, base58, and `id.json` import/export; Bouncy Castle signing and verification with an additional small-order-`R` rejection; typed 64-byte signatures; honest-signature and RFC/upstream vectors | `PublicKey.Verify` is not a consensus-acceptance predicate for adversarial point encodings: it differs from pinned `verify_dalek` on 154/914 pathological C2SP CCTV vectors (63 accepted only by SolSharp, 91 accepted only by dalek; ordinary signatures are unaffected). A dalek-exact managed prototype was intentionally not shipped because validation showed severe CPU and allocation amplification suitable for denial-of-service abuse. |
...
| Signed off-chain messages | Wire-compatible; stricter parser | version-0 domain, restricted/limited/extended UTF-8 formats, bounded parse, hash, sign, verify, exact upstream vectors | Safe-direction difference: `Deserialize` validates the 16 signing-domain bytes, while the pinned decoder checks length and version but does not inspect that domain |
...
| Transaction envelope | Wire-compatible; stricter parser | version-routed legacy/v0/V1 serialization, typed signature slots, partial/full/external signing, per-slot verification, message hash, bounded allocation | Safe-direction difference: `Deserialize` rejects trailing bytes that pinned `wincode::deserialize` ignores |
...
| Native loaders, feature gate, and precompiles | Runtime-compatible | legacy/upgradeable/V4 loader clients and raw runtime states; feature activation/revocation; Ed25519/Secp256k1/Secp256r1 self-contained and offset-table clients | Declared decoder difference: upgradeable-loader `None` authorities retain the runtime's fixed Buffer/ProgramData payload offsets 37/45 rather than the compact bincode/`jsonParsed` client offsets 5/13 |
